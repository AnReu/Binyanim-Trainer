...
function get_new_verb(tenses_list, binyanim_list, verb_list){
    ...
    verb_list_filtered = filter_verb_list(tenses_list, selected_binyan, verb_list)
...
function filter_go_to_verbs(verb_list){
    go_to_verbs = ['להרגיש', 'לדבר', 'להתלבש', 'לגור', 'לקנות', 'להיגמר', 'לסגור', 'לשבת']
    
    
    verb_list_filtered = verb_list.filter((elem)=>{
        return go_to_verbs.indexOf(elem['infinitive_no_niqqud']) >= 0
    })
